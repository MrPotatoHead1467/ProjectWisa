...
if (isset($_POST['Loopbaan_Opslaan'])){
    ...
    if (mysqli_query($conn, $sqlLoopbaan)){
        ...
        if (isset($_FILES["Attest_Doc"])){
            if (!file_exists($Dir)) {
                mkdir($Dir, 0777, true);
            }
            
            $Bestand = $_FILES["Attest_Doc"];
            ...
            $Bestand_Naam = $Bestand_Basename."_Attest_".$Persoon_Id.".".$Soort_Bestand;
            ...
            if ($Bestand['size'] != 0){
                if (move_uploaded_file($Bestand["tmp_name"], $Bestand_Locatie)) {
                    ...
                    if (mysqli_query($conn, $sqlBestanden)){
                        ...
                        $sqlDoc_link = "INSERT INTO tbl_docs_links (fld_doc_id_fk, fld_persoon_id_fk, fld_loopbaan_id_fk)
                                        VALUES ('".$Doc_Id."', '".$Persoon_Id."', '".$Loopbaan_Id."')";
